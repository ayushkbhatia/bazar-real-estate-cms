...
async function main() {
  ...
  /*
   * The gate runs on whatever model the generator runs on, so its properties
   * have to be re-measured per provider. A weaker comparator is a WEAKENED
   * GATE, and the failure is silent: it passes bad Arabic instead of blocking
   * it. That is the one direction this whole harness is built not to fail in,
   * which makes re-running this the price of changing provider.
   */
  const { mtClientFromEnv } = await import("../../lib/i18n/mt/hf-client");
  const { client, fastModel, provider } = await mtClientFromEnv();
  console.log(`Provider: ${provider} · ${fastModel}\n`);
  ...
  console.log("\nScoring nulls…");
  const nullVerdicts = await equivalence({ client, model: fastModel, pairs: nullPairs });
    ...
    `${JSON.stringify(
      {
        ...
        // Which provider produced these numbers. The gate runs on whatever the
        // generator runs on, so a figure without a provider beside it is a
