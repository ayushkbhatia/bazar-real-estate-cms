...
} from "@/app/[locale]/(public)/_components/home/section-copy";
import { OFFPLAN_LAUNCH_COUNT } from "@/app/[locale]/(public)/_components/marketing/counts";
import {
  AREAS_CARD_COUNT,
  MARQUEE_SLUGS,
...
async function fetchMedia(): Promise<MediaOption[]> {
  ...
  const { data } = await supabase
    ...
    .is("deleted_at", null)
    // Images for image fields, PDFs for file fields (the brochure), video for
    // the hero. The editor filters by mime per field, so all three live in one
    // list.
    .or(
