...
const nextConfig: NextConfig = {
   ...
   * a page making sequential reads, each of which may spend up to its 75s
   * retry budget (`BUDGET_MS`), has to be able to finish inside it —
   * `lib/supabase/resilient-fetch.test.ts` reads this value and checks that
   * two such reads fit, so change them together. Raising the
