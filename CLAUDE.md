...
| Inngest | Vercel Cron + Bearer secret (9 jobs) | Jobs are idempotent and fit one execution; durability not yet load-bearing. **Known gap**: silent-failure surface grew with minute-cadence crons. | [ADR-0003](docs/decisions/ADR-0003-vercel-cron-over-inngest.md) |
...
| Fonts | Instrument Serif (display), Geist (UI), JetBrains Mono (data) — `next/font/google`; Arabic is IBM Plex Sans Arabic by default and CMS-overridable per role (`/admin/settings/typography`) |
...
lib/
  forms/                   # Forms Manager: registry of every public lead form,
                           ...
                           #   submission mapping, and `overrides.ts` — the
                           #   diff a `control: "labels"` form hands its own
                           #   bespoke component, plus the registry pin its
                           #   saves are held to. See docs/FORMS.md.
  ...
  content-assets/          # Content Assets: the token vocabulary and its
                           ...
                           #   every email the site sends (17 rewritable + the
                           #   advisor reply), each bound to its built-in
                           #   template and a sample (system-emails.ts) so the
                           #   admin preview IS the send path. Bodies are
                           #   sanitised rich text (email-html.ts); the logo,
                           #   colours and footer come from
                           #   `site_settings.email_branding` (email-brand.ts)
                           #   and wrap built-ins too. `form_reply` assets
                           #   (0128) are whole emails an editor assigns to
                           #   public forms via `forms.reply_asset_id`;
                           #   usage.ts derives, from the forms registry, which
                           #   page and form sends each email. Resolution is
                           #   assigned-reply-wins, then published-row,
                           #   then code template — so a draft (or an
                           #   unreadable row) sends the built-in copy.
                           #   Arabic is a twin column per asset (0129), chosen
                           #   by the LEAD's locale at send time, RTL shell and
                           #   panels included; English is the floor when a
                           #   twin is unwritten. See docs/CONTENT_ASSETS.md.
  card-labels.ts           # Card labels: the vocabulary of chips a property or
                           #   development card wears over its image, plus the
                           #   rule for which of them one listing shows. The
                           #   vocabulary is CMS-editable
                           #   (`site_settings.card_labels`,
                           #   /admin/settings/card-labels); assignment lives in
                           #   `properties.flags.labels` and
                           #   `developments.meta.labels`, both free-form jsonb
                           #   that already existed. The two legacy booleans
                           #   (`exclusive`, `vacant_on_transfer`) are read as
                           #   assignments, so nothing regresses untagged.
  preferences/unit-labels.ts
                           # The site-wide currency + area-unit dictionary:
                           #   "AED"/"ft²" and their Arabic equivalents, keyed
                           #   by locale, overridable per client from
                           #   /admin/settings/units. Every label-emitting
                           #   formatter reads it; the label-free maths ones
                           #   (convertArea, toFt2, toAed) do not. English
                           #   leads a figure, Arabic trails it, and that rule
                           #   is derived from the locale rather than stored.
  master-pages/library.ts  # Section library: content owned by the SITE, not by
                           #   a page — client testimonials, the partner
                           #   ecosystem, the shortlist drawer's copy and the
                           #   compare page's. One document each, read by every
                           #   surface that places them (the partner list feeds
                           #   /partners, /about and the home logo strip), so an
                           #   edit lands everywhere at once.
                           #   Edited at /admin/pages/sub/section.
  partners/                # The banking and regulatory institutions as they
                           #   ship in code: the catalogue the partner-ecosystem
                           #   section seeds itself from, plus the logo
                           #   resolver — an editor's upload wins, then the PNG
                           #   in /public/partners matched on slug or name, then
                           #   the card sets the name in type.
  ...
  master-pages/development-page.ts
                           # Project pages: the eyebrow, heading and standfirst
                           #   above every band on every /developments/<slug>
                           #   page — the words that are the same on all of
                           #   them. One document, `{name}` `{area}`
                           #   `{developer}` `{plan}` `{available}` `{total}`
                           #   substituted per page, so a newly created project
                           #   reads correctly with nothing typed. A project's
                           #   own override still wins, and the per-project
                           #   editor shows this wording greyed in each empty
                           #   box. Edited at
                           #   /admin/pages/sub/development/copy.
  master-pages/property-page.ts
                           # Listing pages: the eyebrow and heading above each
                           #   band on every /p/<slug> page, the enquiry card
                           #   and dialog, the advisor card's label and button,
                           #   and the FAQ questions every listing shares. One
                           #   document, `{reference}` `{title}` `{area}`
                           #   `{advisor}` `{type}` substituted per listing and
                           #   bidi-isolated on /ar. Edited at
                           #   /admin/pages/sub/property.
  master-pages/cards.ts    # Cards: card-shaped blocks that repeat across many
                           #   pages — the project advisor card (pull quote,
                           #   buttons), the listing advisor and enquiry cards.
                           #   A card is a LENS onto a section of an existing
                           #   shared-copy document, not a second store: saving
                           #   one splices that section back in. Adds a live
                           #   preview of the real component (EN/AR, any
                           #   project) and a report of projects whose own
                           #   wording beats the card, with a one-click hand-back
                           #   for the ones that match it word for word.
                           #   Edited at /admin/pages/cards.
...
- **Admin CMS** — properties (publishability gates, agent assignment, bulk reassign/off-market/archive), developments, deals (Kanban stages, documents, KYC), enquiries (auto-reply + escalation crons, threads, assignment, admin-only archive with restore), valuations, audit log + bulk-operations view, users + roles, settings (integrations panel for Meilisearch / Mapbox / Mailchimp / Resend; **Mortgage** — the calculator's opening scenario, closing-cost percentages, Central Bank LTV tiers and DBR thresholds; **Units & currency** — the words prices and areas are written with in Arabic, overriding the shipped "درهم" / "قدم²"; **Card labels** — the vocabulary of chips a property or development card wears over its photograph, each with an Arabic twin and one of five colours, ticked per listing in the property wizard, the property edit rail and a development's CMS page; **Property fields** — the amenity taxonomy, every row with an editable Arabic twin, which is the only place an amenity's Arabic can live because listings store the English word; the property and development editors write new amenities straight into it), pages, blog, analytics with PDF export, **Forms Manager** (every public lead form's visibility, copy, CTA, field list/types/order + its responses — `/admin/forms`; the home hero's search bar, tabs and all, at `/admin/forms/search-bar`; forms a bespoke component draws are `control: "labels"` — every question, placeholder and answer editable, the layout not, starting with the `/services/sell` owner wizard; the message a form's box opens already holding — "I'd like to know more about {reference}." — is copy with an Arabic twin too), **Page Builder** (campaign landing pages assembled from pre-built sections, draft/preview/publish, served at `/lp/<slug>` — `/admin/page-builder`), **Footer** (the site footer's columns, links, socials, contact entries and legal line, each with an Arabic twin — `/admin/footer`), **Sections** (site-wide content edited once and rendered wherever it is placed, at `/admin/pages/sub/section` — client testimonials, the **partner ecosystem** (every bank and regulator's name, relationship line, group and logo, each with an Arabic twin, feeding /partners and the logo strip on the home page and /about), the shortlist drawer and the compare page), **Cards** (the card-shaped blocks every project or listing page repeats — the project advisor card's eyebrow, pull quote and buttons, and the listing advisor and enquiry cards — each with its Arabic, a live preview of the real card for any project in either language, and the list of projects whose own wording overrides it — `/admin/pages/cards`), **Search results** (the eyebrow, headline and sub-title above the filter bar on each of the six search routes, plus the title and description each publishes to a search engine, each with an Arabic twin — `/admin/pages/sub/search`), **Arabic type** (upload the client's licensed Arabic faces and assign one to each of four typographic roles — headings, body/UI, eyebrow labels, numerals — plus a weight for the headings and eyebrow roles, so a family's Bold can be the one headings are drawn in; restyles every Arabic page at once, with a kill switch back to the shipped face — `/admin/settings/typography`), **Developer page copy** (the crumb, both section headings, both buttons and the empty state shared by every `/developers/<slug>` profile, with a `{name}` token and an Arabic twin — `/admin/pages/sub/developer/copy`), **Project page copy** (the eyebrow, heading and standfirst above all fifteen bands on every `/developments/<slug>` page, each with an Arabic twin and tokens for the project's name, area, developer, payment-plan name and unit counts; already filled in, so a newly created project needs nothing typed, and the per-project editor shows the shared wording greyed in each empty box — `/admin/pages/sub/development/copy`), **Property page copy** (the eyebrow and heading above every band on every `/p/<slug>` listing, the enquiry card and the dialog "Send to advisor" opens, the lead-advisor card's label and button, the valuation prompt, the FAQ heading and the questions shown on every listing, and the nearby-listings rail — each with an Arabic twin and tokens for the listing's reference, title, area, advisor and property type; the FAQ entries written from a listing's own facts stay in the catalogue because they carry bedroom and bathroom counts — `/admin/pages/sub/property`), **Content assets** (`/admin/content-assets` — *Site emails*, a gallery of all eighteen emails the site sends, each rendered for real in an iframe and seventeen of them rewritable in a rich-text editor with buttons, images and fields, a live desktop/phone preview and a send-me-a-test button, each overriding its built-in template only while published; *Form replies*, every one of the 22 public forms mapped to the email its visitor receives — with the page and path it sits on — and a reply an editor writes once and assigns to as many forms as they like; *Outreach*, the email and WhatsApp copy an advisor sends by hand from the enquiry composer; and *Email design*, the logo, colours and footer every email — built-in or rewritten — is wrapped in; every screen has an **EN | العربية** toggle, and a lead who filled in an Arabic form is answered from the Arabic twin, right-to-left, falling back to the English when none is written).
- **Compliance** — PDPL DSR export + delete flows, cookie consent banner, all legal pages (`/legal/privacy|terms|cookies`).
- **Integrations** — Meilisearch sync, Voyage embeddings backfill, Mapbox geocoding + isochrones, Mailchimp two-way sync via webhook, Sentry, PostHog (consent-gated, with sign-in identify), Vercel Analytics, syndication push to portals, DLD weekly import, BRN validation, permit expiry alerts, **Salesforce** both ways — every enquiry out to `Lead__c`, every listing the CRM publishes in to `properties`, held with reasons, approved, hidden and mapped at `/admin/properties/salesforce` (docs/SALESFORCE.md).
- **Infra** — 132 migrations, 80+ vitest specs, Playwright specs, 9 cron jobs, full CI gate.
  **Cron status**: `CRON_SECRET` is set in Production + Preview as of 23 Sept 2026, so the Vercel jobs run. Failures are visible at **/admin/settings/health** — `lib/observability.ts` writes deduplicated errors to `error_events` and every job stamps `cron_heartbeats`, so a job that has stopped shows as late without anything needing to send a message (0134). A daily digest at 07:00 emails every active admin (or only the addresses in `HEALTH_DIGEST_RECIPIENTS` when set), but only when there is something to report. Sentry is optional: set a DSN and every report forwards there as well. One gap remains: the pg_net → Supabase Edge Function path for the enquiry auto-reply (migration 0030) is still undeployed with `app_settings` empty — deliberately, since Vercel Cron already covers that job and deploying both would acknowledge every enquiry twice. See docs/FOLLOWUPS.md.
