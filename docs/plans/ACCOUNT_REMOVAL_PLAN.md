...
```
app/(public)/_components/filter-bar.tsx:21   saveCurrentSearch   from @/app/(account)/_actions
...
```
app/(staff-auth)/admin/login/_form.tsx:10    signInAction  from @/app/(public)/(auth)/_actions
components/brand/cms-user-pile.tsx:14        signOutAction from @/app/(public)/(auth)/_actions
components/brand/public-mega-nav-mobile.tsx:18  signOutAction
components/brand/account-menu.tsx:16            signOutAction
```
`rm -rf "app/(public)/(auth)"` breaks **staff sign-in and sign-out**.

```
app/(admin)/admin/deals/_actions.ts:73       if (!enquiry.account_id) return …
...
```
app/(public)/_components/filter-bar.tsx:21   import { saveCurrentSearch } from "@/app/(account)/_actions";
...
```
app/(staff-auth)/admin/login/_form.tsx:10    import { signInAction, type AuthState } from "@/app/(public)/(auth)/_actions";
components/brand/cms-user-pile.tsx:14        import { signOutAction } from "@/app/(public)/(auth)/_actions";
```

`rm -rf "app/(public)/(auth)"` breaks **staff sign-in and staff sign-out**.
...
- Move `app/(public)/(auth)/_actions.ts` → a group-neutral location (e.g. `app/_actions/auth.ts`). Repoint all four importers: `app/(staff-auth)/admin/login/_form.tsx:10`, `components/brand/cms-user-pile.tsx:14`, `components/brand/account-menu.tsx:16`, `components/brand/public-mega-nav-mobile.tsx:18`, plus the three customer pages. Keep `signInAction`, `signOutAction`, `AuthState` byte-identical in behaviour.
...
- `app/(public)/sso/[provider]/callback/route.ts:21` — same default.
...
**Edit**
- Move `toggleSavedProperty` and `saveCurrentSearch` out of `app/(account)/_actions.ts` into a neutral module (e.g. `app/_actions/saved.ts`). Repoint `components/brand/save-button.tsx:8` and `app/(public)/_components/filter-bar.tsx:21`. Leave `updateSavedSearchAlert` / `deleteSavedSearch` where they are — they die with the tree.
...
- `app/(public)/_components/saved-ids-provider.tsx`, `app/(public)/_components/listing-card-saveable.tsx`, `app/api/saved-property-ids/route.ts`, `lib/queries/saved-ids.ts`.
...
- All six `SavedIdsProvider` / `ListingCardSaveable` mount sites: `app/(public)/page.tsx:194,204`, `areas/[slug]/page.tsx:367,377`, `agents/[slug]/page.tsx:347,361`, `_components/curated-grid.tsx:76,86`, `_components/search-list.tsx:112,196,233`, and the docstring in `_components/marketing/map-listing.ts:12`.
- `app/(public)/p/[slug]/_components/action-row.tsx` — 5 controls → 4 (drop Save; Compare/Share/Send-to-advisor are unaffected).
- `app/(public)/_components/filter-bar.tsx:21,68,97-142` — remove the Save-search button and the `isAuthed` read.
- `lib/queries/properties.ts:9-27` — remove `getSavedPropertyIds`; remove its call sites `app/(public)/p/[slug]/page.tsx:22,200,322,609`. **Locked file — needs the §2a exemption.**
...
- `app/(public)/tools/compare/page.tsx:433` and `_components/picker-drawer.tsx:76` — remove the "Open saved properties" links.
...
**Upside to tell the client:** `app/(public)/p/[slug]/page.tsx` declares `revalidate = 60` (:115) but calls `getSessionUser()` (:196), `getSavedPropertyIds()` (:201) and `recordView()` (:209) — cookie reads that force per-request rendering. Removing them lets the PDP genuinely cache. `e2e/buy-cache.spec.ts:8-9` documents that this exact win was already taken for `/buy/search`.
...
**Edit**
- `app/(public)/legal/_layout.tsx:91-96`, `legal/privacy/page.tsx:278,285`, `legal/terms/page.tsx:164`, `app/(public)/data-deleted/page.tsx:24,48` — repoint to the new mechanism. Keep the 30-day response promise (`privacy/page.tsx:310-314`) satisfiable.
- Counsel drafts `docs/drafts/PRIVACY_POLICY_DRAFT.md:139,141`, `TERMS_OF_SERVICE_DRAFT.md:34,128`.
- `e2e/dsr.spec.ts` and `e2e/legal-pages.spec.ts:27,30` — **retarget, do not delete.** These two specs are the tripwire that caught this problem; they earn their keep. Note `e2e/dsr.spec.ts:13-31` is the repo's **only** coverage of the public `app/(public)/data-deleted` page (success copy + `?reason=expired` branch) — preserve that block.
...
**Add**
- Staff "create buyer record" surface inserting an `accounts` row, so `deals.buyer_account_id` (not-null FK) can be satisfied without customer self-signup. Then either relax or re-plumb the gate at `app/(admin)/admin/deals/_actions.ts:73` and the `buyer_account_id` writes at `:98,:124`, plus the conditional Create-deal button at `app/(admin)/admin/enquiries/[id]/page.tsx:226`.
...
- Stale no-op `revalidatePath` calls in surviving code: `app/(admin)/admin/deals/[id]/_actions.ts:328`, `app/(admin)/admin/enquiries/[id]/_actions-viewing.ts:202,244`, `app/(public)/_actions/newsletter.ts:194,234,275`.
- `app/(public)/_actions.ts:72-97` — the auth-aware client that stamps `account_id: user?.id ?? null` becomes always-null dead weight. **Keep the nullable column** (see below); simplify the lookup only if D5 doesn't need it.
- `lib/queries/recently-viewed.ts` — drop `listRecentlyViewed`; keep `recordView` or remove it *together with* its call at `app/(public)/p/[slug]/page.tsx:38,210`.
...
- `app/(public)/data-deleted/page.tsx` — per D2, either keep as the new deletion landing or delete as an orphan.
- Docstring debris: `lib/deals.ts:238`, `lib/hooks/use-session-email.ts:12`, `app/(public)/p/[slug]/_components/schedule-viewing.tsx:14`, `lib/queries/tour-requests.ts:34`, `app/(staff-auth)/admin/login/_form.tsx:19`.

**Deliberately kept:** `lib/queries/enquiries.ts` (staff), `lib/queries/documents.ts`, `lib/documents-storage.ts`, `app/api/documents/[id]/download/route.ts`, `lib/deals.ts`, `lib/queries/deal-gate.ts`, `lib/notifications.ts`, `lib/hooks/use-session-email.ts`, `app/(public)/_actions/newsletter.ts`, `lib/queries/tour-requests.ts::createTourRequest`, `app/api/cron/viewing-reminders` (it notifies the **staff** agent, never emails a customer — the name misleads), `app/api/cron/post-valuation-nurture`.
...
- `/forgot-password` — linked from `app/(staff-auth)/admin/login/_form.tsx:78`.
...
**Recommended:** *move* these four pages into `app/(staff-auth)/` and restyle them as staff surfaces, rather than leaving staff recovery living in a route group named `(auth)` under `(public)`. That is a rename PR, not a delete.

**Delete**
- `app/(public)/(auth)/sign-up/page.tsx` + `signUpAction` — **only after D9's replacement bootstrap exists.** Remove the dead `type='signup'` branch at `app/auth/callback/route.ts:23`, the link at `sign-in/page.tsx:82`, and `"/sign-up"` from `lib/supabase/proxy.ts:26`. Update `supabase/seed.sql:8` and `docs/HANDOVER.md:218`.
- `app/(public)/(auth)/sign-in/page.tsx` per D3, plus a `next.config.ts` permanent redirect `/sign-in → /admin/login`.

**Edit**
- Every `/sign-in` reference: `lib/auth.ts:42`, `lib/supabase/proxy.ts:81`, `app/auth/callback/route.ts:27,30,37`, `app/(public)/sso/[provider]/callback/route.ts:24,32`, `app/(public)/(auth)/reset-password/_actions.ts:57,71`, `app/(public)/data-deleted/page.tsx:48`, `app/(staff-auth)/admin/login/_form.tsx:111`, `components/brand/public-mega-nav-mobile.tsx:249`, `lib/auth.test.ts:56`.
...
- **`enquiries_own_select`** (`0006:188-189`) — **do NOT drop.** It looks like a customer-only read policy but it is the only policy that lets `insert().select("id")` return a row on the authenticated client in `app/(public)/_actions.ts:124-134`. Drop it and the **public enquiry form silently fails** for anyone holding a residual customer session or an invited-but-not-yet-activated staff member. Same mechanism for **`valuation_requests_own_select`** (`0008:126-127`) vs `app/(public)/tools/valuation/_actions.ts:150-162`.
...
- Name-based sweeps are dangerous: `valuation_requests.marketing_opt_in` (`0008_tools.sql:55`) is a **different, live, public-tool** column written by `app/(public)/tools/valuation/_actions.ts:105`.
- `app/(admin)/admin/settings/backups/page.tsx:78` hardcodes the retention string "Saved properties, searches, comparisons, recently viewed" — a **regulatory-facing** admin page that becomes factually wrong. Copy edit, not a compile break.
